...
class ElectronicSmokingPage extends StatelessWidget {
  ElectronicSmokingPage(
      {super.key, this.smokingFrom = SmokingFromOption.electronic});
  ...
  Widget _buildNicotineAmountInputSection(BuildContext context) {
    return SizedBox(
        child: FutureBuilder<List<NicotineAmountEntity>>(
      future: context.read<DataCollectionCubit>().nicotineAmounts,
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return Container();
        } else if (snapshot.hasData) {
          return CollectionDropdown<NicotineAmountEntity>(
            ...
            required: smokingFrom == SmokingFromOption.asthma ||
                ...
                : true,
            hintText: "Selecione",
            value: context.read<DataCollectionCubit>().state.nicotineAmount,
            items: snapshot.data!.map<DropdownMenuItem<NicotineAmountEntity>>(
                (NicotineAmountEntity value) {
              return DropdownMenuItem<NicotineAmountEntity>(
                value: value,
                child: Text(
                  value.name,
            ...
            onChanged: (value) {
              if (value != null) {
                context.read<DataCollectionCubit>().setNicotineAmount(value);
              }
            },
          );
        } else {
  ...
  String getSubtitleByOrigin() {
    switch (smokingFrom) {
      case SmokingFromOption.electronic:
        return "Tabagismo - Eletrônico";
      case SmokingFromOption.both:
  ...
  Widget getSmokingOptionWidget() {
    switch (smokingFrom) {
      ...
      case SmokingFromOption.asthma:
      case SmokingFromOption.control:
        return const SmokingOptionWidget(option: SmokingFromOption.electronic);
      default:
        return Container();
    }
  }
