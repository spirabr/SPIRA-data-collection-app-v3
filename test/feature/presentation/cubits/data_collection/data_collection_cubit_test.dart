...
void main(){
  ...
  setUp(() async {
    ...
    when(() => mockLocalDataSource.getFirstLoginResponse()).thenAnswer((_) async => null);
    when(() => mockConnectivityService.isConnected()).thenAnswer((_) async => false);

    when(() => mockLocalDataSource.insertPatient(
  ...
  group('Testes do novo campo de Monóxido de carbono', () async {
    test('Deve salvar Monóxido de Carbono (Double) para cigarro eletrônico', () async {
      ...
      final capturedCall = verify(() => mockLocalDataSource.insertPatient(
          captureAny(), any(), any(), any(), any(), any(), any(), any(), any(), any()
      ));
